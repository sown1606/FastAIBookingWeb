...
const PushNotificationBridge = () => {
  ...
  useEffect(() => {
    ...
    let active = true;
    void registerFirebaseMessagingToken().then(async () => {
      if (!active) {
        return;
      }
      await subscribeToForegroundMessages((payload) => {
        ...
        const title = payload.notification?.title ?? payload.data?.title ?? "FastAIBooking";
...
const LoginRoute = () => {
  ...
  if (session) {
    return <Navigate to={isCallCenterRole(session.user.role) ? "/call-center" : "/dashboard"} replace />;
  }
  return <LoginPage />;
};
